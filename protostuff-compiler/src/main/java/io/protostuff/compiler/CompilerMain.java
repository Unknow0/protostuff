...
public final class CompilerMain {
	...
	public static List<ProtoModule> loadModules(Properties props, File baseDirForSource, File baseDirForOutput) {
		...
		for (String m : COMMA.split(moduleString)) {
			modules.add(loadModule(props, m.trim(), loader, baseDirForSource, baseDirForOutput, globalOptions, null, null));
		}

		return modules;
	}

	public static ProtoModule loadModule(Properties props, String name, CachingProtoLoader loader, File baseDirForSource, File baseDirForOutput, Properties globalOptions,
		...
		ProtoModule module = newProtoModule(source, output, encoding, outputDir, baseDirForSource, baseDirForOutput);
	...
	public static void addOptionsTo(final Properties target, final String[] options, final Properties config) {
	...
	static void compileProfile(Properties props, String profile, CachingProtoLoader loader, Properties globalOptions, String[] rootProfileOptions, final int nestCount)
			throws Exception {
		...
		for (String m : COMMA.split(moduleString)) {
			...
			if (m.charAt(0) == '@') {
				...
				compileProfile(props, m, loader, globalOptions, nestCount == 0 ? profileOptions : rootProfileOptions, nestCount + 1);
				continue;
			}

			compile(loadModule(props, m, loader, null, null, globalOptions, profileOptions, nestCount == 0 ? profileOptions : rootProfileOptions));
		}
	...
	static void compileWithArgs(final String[] args, int offset, final int limit) throws Exception {
		...
		final CachingProtoLoader loader = ("true".equals(props.getProperty("cache_protos")) || "true".equals(System.getProperty("cache_protos"))) ? new CachingProtoLoader()
				: null;
		...
		for (String arg = propsResource;;) {
			if (offset != limit) {
				...
				if (!new File(arg).exists()) {
					// specific module
					compile(loadModule(props, arg, loader, null, null, globalOptions, null, null));
			...
			for (String m : COMMA.split(moduleString)) {
				...
				compile(loadModule(props, m, loader, null, null, globalOptions, null, null));
			}
