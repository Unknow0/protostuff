...
public class ProtoToProtoCompiler extends STCodeGenerator {
	...
	public static String injectAfterAnnotation(Message extend, Message by, String extendProto, String byContent) {
		...
		if (matcher.find()) {
			...
			extendProto = extendProto.substring(0, openBracketIndex) + LINE_SEPARATOR + indentation + "// " + generateTimestamp(by) + LINE_SEPARATOR
					+ insertIndentation(byContent, indentation) + LINE_SEPARATOR + extendProto.substring(openBracketIndex);
		}
	...
	public static String injectAfterOption(Message extend, Message by, String extendProto, String byContent) {
		...
		if (matcher.find()) {
			...
			matcher.appendReplacement(sb,
					"$1" + "// " + generateTimestamp(by) + LINE_SEPARATOR + indentation + "// $2" + LINE_SEPARATOR + insertIndentation(byContent, indentation));
