...
public final class XmlOutput implements Output, StatefulOutput {
	...
	private static void write(final XMLStreamWriter writer, final String name, final String value) throws IOException {
		try {
	...
	private static void writeB64Encoded(final XMLStreamWriter writer, final String name, final char[] value) throws IOException {
		try {
			writer.writeStartElement(name);
	...
	public void writeByteArray(int fieldNumber, byte[] value, boolean repeated) throws IOException {
		writeB64Encoded(writer, schema.getFieldName(fieldNumber), value.length == 0 ? EMPTY : B64Code.cencode(value));
	}
	...
	public void writeByteRange(boolean utf8String, int fieldNumber, byte[] value, int offset, int length, boolean repeated) throws IOException {
		if (utf8String) {
			...
			writeB64Encoded(writer, schema.getFieldName(fieldNumber), length == 0 ? EMPTY : B64Code.cencode(value, offset, length));
		}
	}
