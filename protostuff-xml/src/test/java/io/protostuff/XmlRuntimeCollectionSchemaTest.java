...
public class XmlRuntimeCollectionSchemaTest extends AbstractRuntimeCollectionSchemaTest {
	...
	protected <T> void roundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] xmlRoundTrip = XmlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuff, 0, protostuff.length), pipeSchema);
		byte[] xmlRoundTripFromStream = XmlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuffStream), pipeSchema);
