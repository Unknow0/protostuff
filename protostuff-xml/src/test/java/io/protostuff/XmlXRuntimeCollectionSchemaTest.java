...
public class XmlXRuntimeCollectionSchemaTest extends AbstractRuntimeCollectionSchemaTest {
	...
	protected <T> void roundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] protostuff = ProtostuffIOUtil.toByteArray(XmlIOUtil.newPipe(xml, 0, xml.length), pipeSchema, buf());

		byte[] protostuffFromStream = ProtostuffIOUtil.toByteArray(XmlIOUtil.newPipe(xmlStream), pipeSchema, buf());
		...
		byte[] xmlRoundTrip = XmlXIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuff, 0, protostuff.length), pipeSchema, buf());
		byte[] xmlRoundTripFromStream = XmlXIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuffStream), pipeSchema, buf());
