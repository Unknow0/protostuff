...
public class XmlPipeTest extends AbstractTest {
	...
	static <T> void protobufRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] xml = XmlIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobuf, 0, protobuf.length), pipeSchema);

		byte[] xmlFromStream = XmlIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobufStream), pipeSchema);
		...
		byte[] protobufRoundTrip = ProtobufIOUtil.toByteArray(XmlIOUtil.newPipe(xml, 0, xml.length), pipeSchema, buf());

		byte[] protobufRoundTripFromStream = ProtobufIOUtil.toByteArray(XmlIOUtil.newPipe(xmlStream), pipeSchema, buf());
	...
	static <T> void protostuffRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] xml = XmlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuff, 0, protostuff.length), pipeSchema);

		byte[] xmlFromStream = XmlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuffStream), pipeSchema);
		...
		byte[] protostuffRoundTrip = ProtostuffIOUtil.toByteArray(XmlIOUtil.newPipe(xml, 0, xml.length), pipeSchema, buf());

		byte[] protostuffRoundTripFromStream = ProtostuffIOUtil.toByteArray(XmlIOUtil.newPipe(xmlStream), pipeSchema, buf());
