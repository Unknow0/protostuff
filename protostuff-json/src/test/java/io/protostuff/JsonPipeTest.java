...
public class JsonPipeTest extends AbstractTest {
	...
	static <T> void protobufRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema, boolean numeric) throws Exception {
		...
		byte[] json = JsonIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobuf, 0, protobuf.length), pipeSchema, numeric);

		byte[] jsonFromStream = JsonIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobufStream), pipeSchema, numeric);
		...
		byte[] protobufRoundTrip = ProtobufIOUtil.toByteArray(JsonIOUtil.newPipe(json, 0, json.length, numeric), pipeSchema, buf());

		byte[] protobufRoundTripFromStream = ProtobufIOUtil.toByteArray(JsonIOUtil.newPipe(jsonStream, numeric), pipeSchema, buf());
		...
		checkCompat(json, protobuf, pipeSchema, numeric, false);
	}
	...
	static <T> void protostuffRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema, boolean numeric) throws Exception {
		...
		byte[] json = JsonIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuff, 0, protostuff.length), pipeSchema, numeric);

		byte[] jsonFromStream = JsonIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuffStream), pipeSchema, numeric);
		...
		byte[] protostuffRoundTrip = ProtostuffIOUtil.toByteArray(JsonIOUtil.newPipe(json, 0, json.length, numeric), pipeSchema, buf());

		byte[] protostuffRoundTripFromStream = ProtostuffIOUtil.toByteArray(JsonIOUtil.newPipe(jsonStream, numeric), pipeSchema, buf());
		...
		checkCompat(json, protostuff, pipeSchema, numeric, true);
	}

	static <T> void checkCompat(byte[] json, byte[] source, Pipe.Schema<T> pipeSchema, boolean numeric, boolean protostuff) throws IOException {
		...
		try {
			JsonXIOUtil.writeTo(out, protostuff ? ProtostuffIOUtil.newPipe(in) : ProtobufIOUtil.newPipe(in), pipeSchema, numeric, buffer);
		} finally {
			buffer.clear();
		}
		...
		byte[] jsonx = JsonXIOUtil.toByteArray(protostuff ? ProtostuffIOUtil.newPipe(source, 0, source.length) : ProtobufIOUtil.newPipe(source, 0, source.length), pipeSchema,
				numeric, buf());
