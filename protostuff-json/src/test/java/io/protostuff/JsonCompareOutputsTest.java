...
public class JsonCompareOutputsTest extends TestCase {
	...
	static final Bar negativeBar = new Bar(-12, "negativeBar", negativeBaz, Bar.Status.STARTED, null, true, -130.031f, -1000.0001d, -101010101);
	...
	public static final Foo foo = SerializableObjects.newFoo(new Integer[] { 90210, -90210, 0 },
			new String[] { "a\u0008\u0009\u000B\r\n\f\t\b\"\\", "ef", "gh", "12345678901234567890123456789012345678901234567890" }, new Bar[] { bar, negativeBar },
			new EnumSample[] { EnumSample.TYPE0, EnumSample.TYPE2 }, new ByteString[] { ByteString.copyFromUtf8("byte1"), ByteString.copyFromUtf8("byte2") },
	...
	public void testBenchmark() throws Exception {
		...
		PrintStream out = dir == null ? System.out
				: new PrintStream(new FileOutputStream(new File(new File(dir), "protostuff-json-" + System.currentTimeMillis() + ".txt"), true));
	...
	public static final Serializer JSON_BUFFERED_OUTPUT = new Serializer() {
		...
		public <T extends Message<T>> byte[] serialize(T message) {
			try {
				return JsonXIOUtil.toByteArray(message, message.cachedSchema(), false, buffer);
			} finally {
				buffer.clear();
			}
		}
	...
	public static final Serializer JSON_BUFFERED_OUTPUT_NUMERIC = new Serializer() {
		...
		public <T extends Message<T>> byte[] serialize(T message) {
			try {
				return JsonXIOUtil.toByteArray(message, message.cachedSchema(), true, buffer);
			} finally {
				buffer.clear();
			}
		}
	...
	static final Serializer[] JSON_SERIALIZERS = new Serializer[] { JSON_OUTPUT, JSON_OUTPUT_NUMERIC, JSON_BUFFERED_OUTPUT, JSON_BUFFERED_OUTPUT_NUMERIC, JSON_STREAMED_OUTPUT,
			JSON_STREAMED_OUTPUT_NUMERIC };
