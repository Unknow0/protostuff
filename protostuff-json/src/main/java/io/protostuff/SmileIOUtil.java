...
public final class SmileIOUtil {
	 ...
	 */
	public static SmileParser newSmileParser(InputStream in, byte[] buf, int offset, int limit) {
	 ...
	 * Creates a {@link SmileParser} from the inputstream with the supplied buf {@code inBuffer} to use.
	 */
	static SmileParser newSmileParser(InputStream in, byte[] buf, int offset, int limit, boolean bufferRecyclable, IOContext context) {
		return new SmileParser(context, DEFAULT_SMILE_FACTORY.getParserFeaturesImpl(), DEFAULT_SMILE_FACTORY.getSmileParserFeaturesImpl(), DEFAULT_SMILE_FACTORY.getCodec(),
				DEFAULT_SMILE_FACTORY.getRootByteSymbols().makeChild(1), in, buf, offset, limit, bufferRecyclable);
	}
	...
	public static SmileGenerator newSmileGenerator(OutputStream out, byte[] buf) {
		return newSmileGenerator(out, buf, 0, false, new IOContext(DEFAULT_SMILE_FACTORY._getBufferRecycler(), out, false));
	}
	...
	static SmileGenerator newSmileGenerator(OutputStream out, byte[] buf, int offset, boolean bufferRecyclable, IOContext context) {
		return new SmileGenerator(context, DEFAULT_SMILE_FACTORY.getGeneratorFeaturesImpl(), DEFAULT_SMILE_FACTORY.getSmileGeneratorFeatures(),
				DEFAULT_SMILE_FACTORY.getCodec(), out, buf, offset, bufferRecyclable);
	}
	...
	public static <T> void writeTo(OutputStream out, T message, Schema<T> schema, boolean numeric) throws IOException {
		...
		final SmileGenerator generator = newSmileGenerator(out, context.allocWriteEncodingBuffer(), 0, true, context);
