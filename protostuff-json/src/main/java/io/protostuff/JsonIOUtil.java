...
public final class JsonIOUtil {
	 ...
	 */
	public static Pipe newPipe(final JsonParser parser, boolean numeric) {
	 ...
	 * Creates a {@link UTF8StreamJsonParser} from the inputstream with the supplied buf {@code inBuffer} to use.
	 */
	static UTF8StreamJsonParser newJsonParser(InputStream in, byte[] buf, int offset, int limit, boolean bufferRecyclable, IOContext context) {
		return new UTF8StreamJsonParser(context, DEFAULT_JSON_FACTORY.getParserFeaturesImpl(), in, DEFAULT_JSON_FACTORY.getCodec(),
				DEFAULT_JSON_FACTORY.getRootByteSymbols().makeChild(1), buf, offset, limit, bufferRecyclable);
	}
	...
	static UTF8JsonGenerator newJsonGenerator(OutputStream out, byte[] buf, int offset, boolean bufferRecyclable, IOContext context) {
		...
		return new UTF8JsonGenerator(context, DEFAULT_JSON_FACTORY.getGeneratorFeaturesImpl(), DEFAULT_JSON_FACTORY.getCodec(), out, buf, offset, bufferRecyclable);
	}
