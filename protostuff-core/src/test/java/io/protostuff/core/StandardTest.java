...
import static io.protostuff.core.SerializableObjects.negativeBaz;
...
public abstract class StandardTest extends AbstractTest {
	...
	public void testFoo() throws Exception {
		...
		byte[] output = toByteArray(fooCompare, Foo.getSchema());
	...
	public void testBar() throws Exception {
		for (Bar barCompare : new Bar[] { bar, negativeBar }) {
			...
			byte[] output = toByteArray(barCompare, Bar.getSchema());
			mergeFrom(output, 0, output.length, dbar, Bar.getSchema());
	...
	public void testBaz() throws Exception {
		for (Baz bazCompare : new Baz[] { baz, negativeBaz }) {
			...
			byte[] output = toByteArray(bazCompare, Baz.getSchema());
			mergeFrom(output, 0, output.length, dbaz, Baz.getSchema());
	...
	public void testEmptyFoo() throws Exception {
		...
		byte[] output = toByteArray(foo, Foo.getSchema());

		Foo parsedFoo = new Foo();
		mergeFrom(output, 0, output.length, parsedFoo, Foo.getSchema());
	}
	...
	public void testEmptyBar() throws Exception {
		...
		byte[] output = toByteArray(bar, Bar.getSchema());

		Bar parsedBar = new Bar();
		mergeFrom(output, 0, output.length, parsedBar, Bar.getSchema());
	}
