...
public class ProtobufBufferedOutputTest extends SerDeserTest {
	...
	public void testNestedOverflow() {
		...
		byte[] data = ProtobufIOUtil.toByteArray(bar, Bar.getSchema(), buf(bufSize));
		...
		data = ProtobufIOUtil.toByteArray(bar, Bar.getSchema(), buf(bufSize));
		assertEquals(bufSize + 11, data.length);

		final Bar parsedBar = new Bar();
		ProtobufIOUtil.mergeFrom(data, parsedBar, Bar.getSchema());
