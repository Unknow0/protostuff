...
public final class SerializableObjects {
	...
	public static final Bar negativeBar = new Bar(-12, "negativeBar", negativeBaz, Bar.Status.STARTED, "a1".getBytes(StandardCharsets.UTF_8), true, -130.031f, -1000.0001d,
			-101010101);

	public static final Baz baz = new Baz(567, "baz", 202020202);
	public static final Bar bar = new Bar(890, "bar", baz, Bar.Status.STARTED, "b2".getBytes(StandardCharsets.UTF_8), true, 150.051f, 2000.0002d, 303030303);

	public static final Foo foo = newFoo(new Integer[] { 90210, -90210, 0 }, new String[] { "ab", "cd" }, new Bar[] { bar, negativeBar },
			new EnumSample[] { EnumSample.TYPE0, EnumSample.TYPE2 }, new byte[][] { "ef".getBytes(StandardCharsets.UTF_8), "gh".getBytes(StandardCharsets.UTF_8) },
			new Boolean[] { true, false }, new Float[] { 1234.4321f, -1234.4321f, 0f }, new Double[] { 12345678.87654321d, -12345678.87654321d, 0d },
			new Long[] { 7060504030201l, -7060504030201l, 0l });

	public static Foo newFoo(Integer[] someInt, String[] someString, Bar[] someBar, EnumSample[] someEnum, byte[][] someBytes, Boolean[] someBoolean, Float[] someFloat,
			Double[] someDouble, Long[] someLong) {

		return new Foo(someInt == null ? null : Arrays.asList(someInt), someString == null ? null : Arrays.asList(someString), someBar == null ? null : Arrays.asList(someBar),
				someEnum == null ? null : Arrays.asList(someEnum), someBytes == null ? null : Arrays.asList(someBytes),
				someBoolean == null ? null : Arrays.asList(someBoolean), someFloat == null ? null : Arrays.asList(someFloat),
				someDouble == null ? null : Arrays.asList(someDouble), someLong == null ? null : Arrays.asList(someLong));
	}
