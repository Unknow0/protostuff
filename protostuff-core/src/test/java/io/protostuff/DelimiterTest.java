...
public abstract class DelimiterTest extends AbstractTest {
	 ...
	 * given schema.
	 */
	protected abstract <T> int writeDelimitedTo(OutputStream out, T message, Schema<T> schema, LinkedBuffer buffer) throws IOException;

	/**
	 * Deserializes from the byte array and data is merged/saved to the message.
	 */
	protected abstract <T> void mergeDelimitedFrom(InputStream in, T message, Schema<T> schema, LinkedBuffer buffer) throws IOException;

	/**
	 * Serializes the {@code message} (delimited) into an {@link OutputStream} via {@link DeferredOutput} using the
	 * given schema.
	 */
	protected abstract <T> int optWriteDelimitedTo(OutputStream out, T message, Schema<T> schema, LinkedBuffer buffer) throws IOException;

	/**
	 * Deserializes from the byte array and data is merged/saved to the message.
	 */
	protected abstract <T> boolean optMergeDelimitedFrom(InputStream in, T message, Schema<T> schema, LinkedBuffer buffer) throws IOException;
	...
	public void testBarTooLarge2() throws Exception {
		Schema<Bar> schema = Bar.getSchema();
		Bar message = new Bar();
		message.setSomeBytes(ByteString.wrap(new byte[StringSerializer.THREE_BYTE_LOWER_LIMIT - 1]));
	...
	public void testBarTooLarge3() throws Exception {
		Schema<Bar> schema = Bar.getSchema();
		Bar message = new Bar();
		message.setSomeBytes(ByteString.wrap(new byte[StringSerializer.FOUR_BYTE_LOWER_LIMIT - 1]));
