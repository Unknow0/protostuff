...
public class CompareOutputsTest extends AbstractTest {
	...
	static final Bar negativeBar = new Bar(-12, "negativeBar", negativeBaz, Bar.Status.STARTED, ByteString.copyFromUtf8("a1"), true, -130.031f, -1000.0001d, -101010101);

	static final Baz baz = new Baz(567, "baz", 202020202);
	static final Bar bar = new Bar(890, "bar", baz, Bar.Status.STARTED, ByteString.copyFromUtf8("b2"), true, 150.051f, 2000.0002d, 303030303);

	// a total of 1000 bytes
	public static final Foo foo = SerializableObjects.newFoo(new Integer[] { 90210, -90210, 0 }, new String[] { "foo", "123456789012345678901234567890123456789012" },
			new Bar[] { bar, negativeBar }, new EnumSample[] { EnumSample.TYPE0, EnumSample.TYPE2 },
	...
	public void testBenchmark() throws Exception {
		...
		PrintStream out = dir == null ? System.out
				: new PrintStream(new FileOutputStream(new File(new File(dir), "protostuff-core-" + System.currentTimeMillis() + ".txt"), true));
	...
	public static <T extends Message<T>> void start(T message, Serializer[] serializers, PrintStream out, int warmups, int loops) throws Exception {
		for (Serializer s : serializers) {
			ser(message, s, out, s.getName(), warmups, loops);
		}
	}

	static <T extends Message<T>> void ser(T message, Serializer serializer, PrintStream out, String name, int warmups, int loops) throws Exception {
	...
	static final Serializer[] SERIALIZERS = new Serializer[] { PROTOBUF_COMPUTED_OUTPUT, PROTOBUF_BUFFERED_OUTPUT, PROTOBUF_STREAMED_OUTPUT, PROTOSTUFF_COMPUTED_OUTPUT,
			PROTOSTUFF_BUFFERED_OUTPUT, PROTOSTUFF_STREAMED_OUTPUT };
