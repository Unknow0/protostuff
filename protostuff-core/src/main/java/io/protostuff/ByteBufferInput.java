...
public final class ByteBufferInput implements Input {
	...
	public long readRawLittleEndian64() {
		...
		return (((long) bs[0] & 0xff)) | (((long) bs[1] & 0xff) << 8) | (((long) bs[2] & 0xff) << 16) | (((long) bs[3] & 0xff) << 24) | (((long) bs[4] & 0xff) << 32)
				| (((long) bs[5] & 0xff) << 40) | (((long) bs[6] & 0xff) << 48) | (((long) bs[7] & 0xff) << 56);
	}
	...
	public void transferByteRangeTo(Output output, boolean utf8String, int fieldNumber, boolean repeated) throws IOException {
		...
		if (utf8String) {
			...
			if (buffer.hasArray()) {
				output.writeByteRange(true, fieldNumber, buffer.array(), buffer.arrayOffset() + buffer.position(), length, repeated);
				buffer.position(buffer.position() + length);
			} else {
