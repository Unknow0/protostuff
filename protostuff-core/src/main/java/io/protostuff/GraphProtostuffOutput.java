...
public final class GraphProtostuffOutput extends FilterOutput<ProtostuffOutput> {
	...
	public <T> void writeObject(int fieldNumber, T value, Schema<T> schema, boolean repeated) throws IOException {
		...
		if (references.shouldIncrement(refCount, value, output, fieldNumber)) {
			...
			output.tail = output.sink.writeVarInt32(makeTag(fieldNumber, WIRETYPE_START_GROUP), output, output.tail);

			schema.writeTo(this, value);

			output.tail = output.sink.writeVarInt32(makeTag(fieldNumber, WIRETYPE_END_GROUP), output, output.tail);
		}
	}
		...
		public boolean shouldIncrement(int value, Object k, WriteSession output, int fieldNumber) throws IOException {
			...
			while ((item = tab[i]) != null) {
				if (item == k) {
					...
					output.tail = output.sink.writeVarInt32(((Integer) tab[i + 1]).intValue(), output,
							output.sink.writeVarInt32(makeTag(fieldNumber, WIRETYPE_REFERENCE), output, output.tail));
