	...
	public static <T> int mergeDelimitedFrom(InputStream in, T message, Schema<T> schema) throws IOException {
		...
		if (len != 0) {
			...
			if (len > CodedInput.DEFAULT_BUFFER_SIZE) {
				...
				final CodedInput input = new CodedInput(new LimitedInputStream(in, len), true);
				final GraphCodedInput graphInput = new GraphCodedInput(input);
				schema.mergeFrom(graphInput, message);
				input.checkLastTagWas(0);
				return len;
			...
			final ByteArrayInput input = new ByteArrayInput(buf, 0, len, true);
			final GraphByteArrayInput graphInput = new GraphByteArrayInput(input);
			try {
				schema.mergeFrom(graphInput, message);
	...
	public static <T> int mergeDelimitedFrom(DataInput in, T message, Schema<T> schema) throws IOException {
		...
		if (len != 0) {
			...
			if (len > CodedInput.DEFAULT_BUFFER_SIZE && in instanceof InputStream) {
				// message too big
				final CodedInput input = new CodedInput(new LimitedInputStream((InputStream) in, len), true);
				final GraphCodedInput graphInput = new GraphCodedInput(input);
				schema.mergeFrom(graphInput, message);
				input.checkLastTagWas(0);
			} else {
