...
public final class RuntimeView {
	 ...
	 * Returns a new view schema based from an existing one.
	 */
	public static <T> Schema<T> createFrom(RuntimeSchema<T> ms, Instantiator<T> instantiator, Factory vf, Predicate.Factory pf, String... args) {
	...
	public static abstract class PostFilteredSchema<T> extends BaseSchema<T> {
		...
		protected PostFilteredSchema(Class<? super T> typeClass, Instantiator<T> instantiator, Field<T>[] fields) {
	...
	public enum Factories implements Factory {
		...
		PREDICATE {
			@Override
			public <T> Schema<T> create(final RuntimeSchema<T> ms, Instantiator<T> instantiator, Predicate.Factory pf, String[] args) {
				if (pf == null) {
