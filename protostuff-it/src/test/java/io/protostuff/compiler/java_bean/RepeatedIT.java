...
public class RepeatedIT {
	...
	public void testUnmodifiableList() throws Exception {
		...
		list.mergeFrom(createInput(42), list);
		Assert.assertThrows(UnsupportedOperationException.class, () -> list.mergeFrom(createInput(43), list));
	}

	@SuppressWarnings("unchecked")
	private Input createInput(int result) throws IOException {
		Input input = Mockito.mock(Input.class);
		Mockito.when(input.readFieldNumber(Matchers.any(Schema.class))).thenReturn(1).thenReturn(0);
		Mockito.when(input.readInt32()).thenReturn(result);
		return input;
	}
}
