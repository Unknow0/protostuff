	...
	public static class RpcMethod extends AnnotationContainer implements HasName, HasOptions {
		...
		RpcMethod(String name, Service service, String argName, String argPackage, String retName, String retPackage) {
			...
			if (service.rpcMethods.put(name, this) != null) {
				throw err("Duplicate rpc method: " + name + " from service " + service.name, service.getProto());
			}
		}
		...
		public String getJavaArgType() {
			...
			return getProto().getJavaPackageName().equals(argType.getProto().getJavaPackageName()) ? argType.getRelativeName() : argType.getJavaFullName();
		}
		...
		public String getJavaReturnType() {
			...
			return getProto().getJavaPackageName().equals(returnType.getProto().getJavaPackageName()) ? returnType.getRelativeName() : returnType.getJavaFullName();
		}
		...
		void resolveReferences() {
			...
			String enclosingNs = service.isNested() ? service.parentMessage.getFullName() : proto.getPackageName();
			...
			if (!"void".equals(fullArgName)) {
				...
				if (argType == null) {
					throw err("The message " + fullArgName + " is not defined", proto);
				}
			...
			if (!"void".equals(fullReturnName)) {
				...
				if (returnType == null) {
					throw err("The message " + fullReturnName + " is not defined", proto);
				}
