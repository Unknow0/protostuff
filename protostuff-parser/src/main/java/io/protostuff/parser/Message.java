...
public class Message extends AnnotationContainer implements HasName, HasFields {
	...
	void addNestedMessage(Message message) {
		if (nestedMessages.put(message.name, message) != null) {
			throw err("Duplicate nested message: " + message.name + " from message: " + name, getProto());
		}
	}
	...
	void addNestedEnumGroup(EnumGroup enumGroup) {
		if (nestedEnumGroups.put(enumGroup.name, enumGroup) != null) {
			throw err("Duplicate nested enum: " + enumGroup.name + " from message: " + name, getProto());
		}
	}
	...
	void addNestedService(Service service) {
		if (nestedServices.put(service.name, service) != null) {
			throw err("Duplicate nested service: " + service.name + " from message: " + name, getProto());
		}
	}
	...
	public String toString() {
		return new StringBuilder().append('{').append("name:").append(name).append(',').append("enumGroups:").append(nestedEnumGroups.values()).append(',')
				.append("extensions:").append(nestedExtensions).append(',').append("fields:").append(fields.values()).append('}').toString();
	}
	...
	static EnumField newEnumField(EnumGroup enumGroup, Field.Reference fr, Message owner) {
		...
		if (refName == null) {
			...
			if (ef.defaultValue == null) {
				throw err("The field: " + ef.name + " contains an unknown enum value: " + refName, owner.getProto());
			}
		}
		copy(fr, ef);
