...
public class EnumGroup extends AnnotationContainer implements HasName, HasOptions {
	...
	void add(Value value) {
		if (values.put(value.name, value) != null) {
			throw err("The enum " + name + " has duplicate names: " + value.name, getProto());
		}
	...
	public String toString() {
		return new StringBuilder().append('{').append("name:").append(name).append(',').append("values:").append(values).append('}').toString();
	}
		...
		public static final Comparator<Value> NO_ALIAS_COMPARATOR = new Comparator<Value>() {
			...
			public int compare(Value v1, Value v2) {
				if (v1.number == v2.number) {
					throw err(
							"The enum value " + v2.enumGroup.getName() + "." + v2.getName() + " cannot have the same number as " + v1.enumGroup.getName() + "." + v1.getName(),
