...
public abstract class Field<T> extends AnnotationContainer implements Comparable<Field<?>>, HasName, HasOptions {
	...
	public void putExtraOption(java.lang.String key, Object value) {
		if (extraOptions.put(key, value) != null) {
			...
			throw err("The field " + name + ofOwner + " contains a duplicate option: " + key, proto);
		}
	}
	...
	public java.lang.String toString() {
		return new StringBuilder().append('{').append("type:").append(getClass().getSimpleName()).append(',').append("name:").append(name).append(',').append("number:")
				.append(number).append(',').append("modifier:").append(modifier).append(',').append("packable:").append(packable).append(',').append("defaultValue:")
	...
	public int compareTo(Field<?> f) {
		if (f.number == number) {
			throw err("The field: " + f.name + " cannot have the same number as " + name, owner == null ? null : owner.getProto());
		}
