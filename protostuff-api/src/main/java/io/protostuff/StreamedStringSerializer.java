...
public final class StreamedStringSerializer {
	 ...
	 */
	public static LinkedBuffer writeInt(final int value, final WriteSession session, LinkedBuffer lb) throws IOException {
	 ...
	 */
	public static LinkedBuffer writeLong(final long value, final WriteSession session, LinkedBuffer lb) throws IOException {
	 ...
	 * buffer
	 */
	public static LinkedBuffer writeFloat(final float value, final WriteSession session, final LinkedBuffer lb) throws IOException {
	 ...
	 * buffer
	 */
	public static LinkedBuffer writeDouble(final double value, final WriteSession session, final LinkedBuffer lb) throws IOException {
	 ...
	 */
	public static LinkedBuffer writeUTF8(final CharSequence str, final WriteSession session, final LinkedBuffer lb) throws IOException {
	 ...
	 */
	public static LinkedBuffer writeAscii(final CharSequence str, final WriteSession session, final LinkedBuffer lb) throws IOException {
		final int len = str.length();
		if (len == 0)
			return lb;
		...
		if (offset + len > limit) {
			// need to flush
			int index = 0, start = lb.start, bufSize = limit - start, available = limit - offset, remaining = len - available;
	...
	private static void flushAndReset(LinkedBuffer node, final WriteSession session) throws IOException {
		int len;
		do {
	 ...
	 */
	public static LinkedBuffer writeUTF8FixedDelimited(final CharSequence str, final WriteSession session, LinkedBuffer lb) throws IOException {
	 ...
	 */
	public static LinkedBuffer writeUTF8FixedDelimited(final CharSequence str, final boolean littleEndian, final WriteSession session, final LinkedBuffer lb)
			throws IOException {
		...
		if (withIntOffset + len > lb.buffer.length) {
			...
			if (withIntOffset + len > lb.buffer.length) {
				...
				// slow path
				final LinkedBuffer rb = StringSerializer.writeUTF8(str, 0, len, lb.buffer, withIntOffset, lb.buffer.length, session, lb);
	...
	private static LinkedBuffer writeUTF8OneByteDelimited(final CharSequence str, final int index, final int len, final WriteSession session, final LinkedBuffer lb)
			throws IOException {
	...
	private static LinkedBuffer writeUTF8VarDelimited(final CharSequence str, final int index, final int len, final int lowerLimit, int expectedSize,
		...
		if (withIntOffset + len > lb.buffer.length) {
			...
			if (withIntOffset + len > lb.buffer.length) {
				...
				// slow path
				final LinkedBuffer rb = StringSerializer.writeUTF8(str, index, len, lb.buffer, withIntOffset, lb.buffer.length, session, lb);
		...
		if (size < lowerLimit) {
			...
			if (rb != lb || expectedSize != 2) {
				...
				if (existingOffset == lb.start) {
					...
					// flush and reset
					lb.offset = session.flush(lb.buffer, lb.start, existingOffset - lb.start, lb.buffer, offset, lb.offset - offset);
				}
	 ...
	 */
	public static LinkedBuffer writeUTF8VarDelimited(final CharSequence str, final WriteSession session, final LinkedBuffer lb) throws IOException {
		final int len = str.length();
		if (len == 0) {
