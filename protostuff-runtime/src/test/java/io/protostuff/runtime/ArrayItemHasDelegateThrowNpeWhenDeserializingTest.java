...
public class ArrayItemHasDelegateThrowNpeWhenDeserializingTest {
	...
	public void testIt() {
		try {
			...
			originMsg.value = new ArrayList<>();
			Calendar[] vArray = new Calendar[1];
			vArray[0] = Calendar.getInstance();
			originMsg.value.add(vArray);

			Schema<ClassThrowNpeWhenDeserializing> schema = RuntimeSchema.getSchema(ClassThrowNpeWhenDeserializing.class, idStrategy);
	...
	static class ClassThrowNpeWhenDeserializing {
		List<Calendar[]> value = new ArrayList<>();
	}
