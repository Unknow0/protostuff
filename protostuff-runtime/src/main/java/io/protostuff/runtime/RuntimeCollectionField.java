...
abstract class RuntimeCollectionField<T, V> extends Field<T> {
	...
	public RuntimeCollectionField(FieldType type, int number, String name, Tag tag, MessageFactory messageFactory, boolean allowNullElement) {
		...
		schema = new CollectionSchema<V>(messageFactory, allowNullElement) {
			...
			protected void writeValueTo(Output output, int fieldNumber, V value, boolean repeated) throws IOException {
				RuntimeCollectionField.this.writeValueTo(output, fieldNumber, value, repeated);
			}

			@Override
			protected void transferValue(Pipe pipe, Input input, Output output, int number, boolean repeated) throws IOException {
				RuntimeCollectionField.this.transferValue(pipe, input, output, number, repeated);
			}
		};
	}
