...
public abstract class PolymorphicPojoCollectionSchema extends PolymorphicSchema {
	...
	static void writeObjectTo(Output output, Object value, Schema<?> currentSchema, IdStrategy strategy) throws IOException {
		final HasSchema<Object> hs = strategy.tryWritePojoIdTo(output, ID_POJO, (Class<Object>) value.getClass(), true);

		if (hs == null) {
			PolymorphicCollectionSchema.writeObjectTo(output, value, currentSchema, strategy);
			return;
		}
