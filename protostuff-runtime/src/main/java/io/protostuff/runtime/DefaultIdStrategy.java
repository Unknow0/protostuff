...
public final class DefaultIdStrategy extends IdStrategy {
	...
	public <T> boolean registerPojo(Class<T> typeClass, Schema<T> schema) {
		...
		final HasSchema<?> last = pojoMapping.putIfAbsent(typeClass.getName(), new Registered<>(schema, this));
	...
	public <T> boolean registerPojo(Class<T> typeClass) {
		...
		final HasSchema<?> last = pojoMapping.putIfAbsent(typeClass.getName(), new LazyRegister<>(typeClass, this));
	...
	public <T> boolean map(Class<? super T> baseClass, Class<T> typeClass) {
		...
		final HasSchema<?> last = pojoMapping.putIfAbsent(baseClass.getName(), new Mapped<>(baseClass, typeClass, this));
	...
	static final class Registered<T> extends HasSchema<T> {
		...
		public Pipe.Schema<T> getPipeSchema() {
			...
			if (pipeSchema == null) {
				synchronized (this) {
					if ((pipeSchema = this.pipeSchema) == null) {
						this.pipeSchema = pipeSchema = RuntimeSchema.resolvePipeSchema(schema, schema.typeClass(), true);
					}
				}
			}
			return pipeSchema;
