...
public abstract class ObjectSchema extends PolymorphicSchema {
	...
	static Object readObjectFrom(final Input input, final Schema<?> schema, Object owner, IdStrategy strategy) throws IOException {
		...
		switch (number) {
			...
			case ID_ARRAY_SCALAR: {
				...
				final ArraySchemas.Base arraySchema = ArraySchemas.getSchema(id, ArraySchemas.isPrimitive(arrayId), strategy);
	...
	static void writeObjectTo(Output output, Object value, Schema<?> currentSchema, IdStrategy strategy) throws IOException {
		...
		final HasDelegate<Object> hd = strategy.tryWriteDelegateIdTo(output, ID_DELEGATE, clazz);
		...
		// pojo
		final Schema<Object> schema = strategy.writePojoIdTo(output, ID_POJO, clazz).getSchema();
	...
	private static boolean isComponentPojo(Class<?> componentType) {
		return Message.class.isAssignableFrom(componentType)
				|| (!Throwable.class.isAssignableFrom(componentType) && !Map.class.isAssignableFrom(componentType) && !Collection.class.isAssignableFrom(componentType));
	}

	private static void writeComponentTo(Output output, Object value, Schema<?> currentSchema, IdStrategy strategy, Class<?> componentType, int dimensions)
			throws IOException {
	...
	@SuppressWarnings("unchecked")
	private static void writeArrayTo(Output output, Object value, Schema<?> currentSchema, IdStrategy strategy, final Class<Object> clazz) throws IOException {
		final Class<?> componentType = clazz.getComponentType();

		final HasDelegate<Object> hdArray = strategy.tryWriteDelegateIdTo(output, ID_ARRAY_DELEGATE, (Class<Object>) componentType);

		if (hdArray != null) {
			if (output instanceof StatefulOutput) {
				// update using the derived schema.
				((StatefulOutput) output).updateLast(hdArray.genericElementSchema, currentSchema);
			}
		...
		final RuntimeFieldFactory<?> inlineArray = RuntimeFieldFactory.getInline(componentType);
		if (inlineArray != null) {
			// scalar
			final boolean primitive = componentType.isPrimitive();
			final ArraySchemas.Base arraySchema = ArraySchemas.getSchema(inlineArray.id, primitive, strategy);

			output.writeUInt32(ID_ARRAY_SCALAR, ArraySchemas.toArrayId(inlineArray.id, primitive), false);
		...
		if (componentType.isEnum() || (null != (c = componentType.getSuperclass()) && c.isEnum())) {
			// enum
			...
			if (output instanceof StatefulOutput) {
				// update using the derived schema.
				((StatefulOutput) output).updateLast(eio.genericElementSchema, currentSchema);
			}
		...
		HasSchema<Object> hs = strategy.tryWritePojoIdTo(output, ID_ARRAY_POJO, (Class<Object>) componentType, false);

		if (hs != null) {
			if (output instanceof StatefulOutput) {
				// update using the derived schema.
				((StatefulOutput) output).updateLast(hs.genericElementSchema, currentSchema);
			}
		...
		// pojo
		hs = strategy.writePojoIdTo(output, ID_ARRAY_POJO, (Class<Object>) componentType);
	...
	static void transferObject(Pipe.Schema<Object> pipeSchema, Pipe pipe, Input input, Output output, IdStrategy strategy) throws IOException {
		...
		switch (number) {
			...
			case ID_DELEGATE: {
				final HasDelegate<Object> hd = strategy.transferDelegateId(input, output, number);
			...
			case ID_ARRAY_DELEGATE: {
				...
				if (output instanceof StatefulOutput) {
					// update using the derived schema.
					((StatefulOutput) output).updateLast(hd.genericElementSchema.getPipeSchema(), pipeSchema);
				}

				Pipe.transferDirect(hd.genericElementSchema.getPipeSchema(), pipe, input, output);
				return;
			}
			...
			case ID_ARRAY_SCALAR: {
				...
				if (output instanceof StatefulOutput) {
					// update using the derived schema.
					((StatefulOutput) output).updateLast(arraySchema.getPipeSchema(), pipeSchema);
				}
			...
			case ID_ARRAY_ENUM: {
				...
				if (output instanceof StatefulOutput) {
					// update using the derived schema.
					((StatefulOutput) output).updateLast(eio.genericElementSchema.getPipeSchema(), pipeSchema);
				}

				Pipe.transferDirect(eio.genericElementSchema.getPipeSchema(), pipe, input, output);
				return;
			}

			case ID_ARRAY_POJO: {
				final HasSchema<Object> hs = strategy.transferPojoId(input, output, number);

				if (output instanceof StatefulOutput) {
					// update using the derived schema.
					((StatefulOutput) output).updateLast(hs.genericElementSchema.getPipeSchema(), pipeSchema);
				}

				Pipe.transferDirect(hs.genericElementSchema.getPipeSchema(), pipe, input, output);
				return;
			}
