...
final class RuntimeRepeatedFieldFactory {
	...
	private static <T> Field<T> createCollectionInlineV(int number, String name, java.lang.reflect.Field f, final MessageFactory messageFactory,
	...
	private static <T> Field<T> createCollectionEnumV(int number, String name, java.lang.reflect.Field f, final MessageFactory messageFactory, final Class<Object> genericType,
					...
					for (Object o : collection) {
						if (o == null) {
							...
							throw new RuntimeException(
									f.getDeclaringClass().getName() + "::" + f.getName() + " - " + o.getClass().getName() + " is not assignable to " + genericType.getName());
						} else {
				...
				if (existing != null && !existing.isEmpty()) {
					for (Object o : existing) {
						if (o == null) {
							...
							throw new RuntimeException(f.getDeclaringClass() + "::" + f.getName() + " - " + o.getClass() + " is not assignable to " + genericType);
						} else {
							output.writeObject(number, o, schema, true);
						}
					}
	...
	private static final RuntimeFieldFactory<Collection<?>> REPEATED = new RuntimeFieldFactory<Collection<?>>(RuntimeFieldFactory.ID_COLLECTION) {
		...
		public <T> Field<T> create(int number, String name, java.lang.reflect.Field f, IdStrategy strategy) {
			...
			if (EnumSet.class.isAssignableFrom(clazz)) {
				...
				return createCollectionEnumV(number, name, f, strategy.getEnumIO(enumType).getEnumSetFactory(), enumType, strategy);
			}

			final MessageFactory messageFactory = strategy.getCollectionFactory(clazz);
			...
			if (genericType == null || ((Map.class.isAssignableFrom(genericType) || Collection.class.isAssignableFrom(genericType)) && !strategy.isRegistered(genericType))) {
				...
				return createCollectionObjectV(number, name, f, messageFactory, genericType, PolymorphicSchemaFactories.OBJECT, strategy);
			}
			...
			final PolymorphicSchema.Factory factory = PolymorphicSchemaFactories.getFactoryFromRepeatedValueGenericType(genericType);
			if (factory != null) {
				return createCollectionObjectV(number, name, f, messageFactory, genericType, factory, strategy);
			}
