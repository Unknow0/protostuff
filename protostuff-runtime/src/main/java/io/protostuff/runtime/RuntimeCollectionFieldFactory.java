...
final class RuntimeCollectionFieldFactory {
	...
	private static <T> Field<T> createCollectionInlineV(int number, String name, java.lang.reflect.Field f, MessageFactory messageFactory, boolean allowNullElement,
			final Delegate<Object> inline) {
		final Accessor accessor = AF.create(f);
		return new RuntimeCollectionField<T, Object>(inline.getFieldType(), number, name, f.getAnnotation(Tag.class), messageFactory, allowNullElement) {
			@Override
	...
	private static <T> Field<T> createCollectionEnumV(int number, String name, java.lang.reflect.Field f, MessageFactory messageFactory, boolean allowNullElement,
	...
	private static <T> Field<T> createCollectionPojoV(int number, String name, final java.lang.reflect.Field f, final MessageFactory messageFactory, boolean allowNullElement,
		...
		return new RuntimeCollectionField<T, Object>(FieldType.MESSAGE, number, name, f.getAnnotation(Tag.class), messageFactory, allowNullElement) {
			...
			protected void writeValueTo(Output output, int fieldNumber, Object value, boolean repeated) throws IOException {
				if (!genericType.isAssignableFrom(value.getClass())) {
					throw new RuntimeException(
							f.getDeclaringClass().getName() + "::" + f.getName() + " - " + value.getClass().getName() + " is not assignable to " + genericType.getName());
				}
			...
			protected Field<T> copy(IdStrategy strategy) {
				return createCollectionPojoV(number, name, f, messageFactory, 0 != (strategy.flags & IdStrategy.PRESERVE_NULL_ELEMENTS), genericType, strategy);
			}
		};
	}
	...
	private static <T> Field<T> createCollectionPolymorphicV(int number, String name, final java.lang.reflect.Field f, final MessageFactory messageFactory,
		...
		return new RuntimeCollectionField<T, Object>(FieldType.MESSAGE, number, name, f.getAnnotation(Tag.class), messageFactory, allowNullElement) {
			...
			@Override
			protected void addValueFrom(Input input, Collection<Object> collection) throws IOException {
				final Object value = input.mergeObject(collection, strategy.POLYMORPHIC_POJO_ELEMENT_SCHEMA);
	...
	private static final RuntimeFieldFactory<Collection<?>> COLLECTION = new RuntimeFieldFactory<Collection<?>>(RuntimeFieldFactory.ID_COLLECTION) {
		...
		public <T> Field<T> create(int number, String name, java.lang.reflect.Field f, IdStrategy strategy) {
			...
			if (0 != (IdStrategy.POJO_SCHEMA_ON_COLLECTION_FIELDS & strategy.flags) && (morph == null || morph.value())) {
			...
			if (EnumSet.class.isAssignableFrom(f.getType())) {
				...
				// TODO optimize
				return createCollectionEnumV(number, name, f, strategy.getEnumIO(enumType).getEnumSetFactory(), 0 != (strategy.flags & IdStrategy.PRESERVE_NULL_ELEMENTS),
						enumType, strategy);
			}
			...
			if (inline != null) {
				return createCollectionInlineV(number, name, f, messageFactory, 0 != (strategy.flags & IdStrategy.PRESERVE_NULL_ELEMENTS), inline);
			}
			...
			if (genericType.isEnum()) {
				return createCollectionEnumV(number, name, f, messageFactory, 0 != (strategy.flags & IdStrategy.PRESERVE_NULL_ELEMENTS), genericType, strategy);
			}
