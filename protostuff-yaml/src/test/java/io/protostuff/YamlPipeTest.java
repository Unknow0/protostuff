...
public class YamlPipeTest extends AbstractTest {
	...
	static <T> void protobufRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] yaml = YamlIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobuf, 0, protobuf.length), pipeSchema, buf());

		byte[] yamlFromStream = YamlIOUtil.toByteArray(ProtobufIOUtil.newPipe(protobufStream), pipeSchema, buf());
	...
	static <T> void protostuffRoundTrip(T message, Schema<T> schema, Pipe.Schema<T> pipeSchema) throws Exception {
		...
		byte[] yaml = YamlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuff, 0, protostuff.length), pipeSchema, buf());

		byte[] yamlFromStream = YamlIOUtil.toByteArray(ProtostuffIOUtil.newPipe(protostuffStream), pipeSchema, buf());
