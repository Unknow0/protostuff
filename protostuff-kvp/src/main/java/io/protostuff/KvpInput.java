...
public final class KvpInput implements Input {
	...
	static final int DEFAULT_BUFFER_SIZE = Integer.getInteger("kvpinput.default_buffer_size", 1024);

	static final int MAX_VALUE_SIZE = Integer.getInteger("kvpinput.max_value_size", 8192);
	...
	private byte[] fill(final byte[] data, int dataOffset, final int len) throws IOException {
	...
	public <T> int readFieldNumber(Schema<T> schema) throws IOException {
		...
		final int number = numeric ? parseInt(buffer, offset, size, 10, true) : schema.getFieldNumber(STRING.deser(buffer, offset, size));
